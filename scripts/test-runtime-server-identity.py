...
with tempfile.TemporaryDirectory(prefix="runtime-server-identity-") as temporary:
    own = Path(temporary) / "own"
    foreign = Path(temporary) / "foreign"
    for directory in (own, foreign):
        directory.mkdir()
        fixtures.seed(directory)
    result = subprocess.run([sys.executable, str(ROOT / "scripts/fixtures/runtime-other-database.py"), str(own), str(foreign)],
