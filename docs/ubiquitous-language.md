...
- 使われ方：通信結果が失われた後の再試行で重複を作らず、同じcapabilityによる完全な候補集合の変更は200で更新する。公開回答 ID は資格情報にならない。
...
- **イベント削除**：主催者capabilityを照合して、イベントと候補・回答・回答選択を一括で消去するstaging操作。system synonym: `DELETE /api/events/:id`。参照: [ADR 0059](ADR/0059-delete-staging-events-by-organizer-capability.md)。
...
- **実 URL smoke 検証**：配置済み staging URL に対し、主要 API 導線を短時間で作成から削除まで再実行する検証。system synonym: real-URL smoke verifier。参照: [ADR 0080](ADR/0080-re-runnable-staging-real-url-smoke.md)。
