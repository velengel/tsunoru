...
- [x] ローカル・実 URL・320px/desktop ブラウザー・実機の証拠を区別し、必要チェックと受領済みレビューの判断を記録する。初回1件は検証・返信・解決済み。最大2往復とし、最終 head の状態は PR 上で確認する。
...
- [x] ローカルレビューと初回 Codex review をバッチで判断し、必要な修正・返信を行う。最終確認は残り1回以内。
