...
- [x] 既存アカウントの専用 Worker・新規 D1 に配置し、実 URL と配置 version の証拠を残す。認証・作成・回答・集計・削除の実 URL journey まで確認済み。
...
- [x] 新規 D1、secret、staging URL を設定し、合成データで認証・作成・回答・集計・削除の実 URL journey を検証する。
- [x] ローカルレビューと2往復の Codex review をバッチで判断し、必要な修正・返信を行う。3回目は依頼しない。

## concern

ローカル画面と各検証の根拠は [report 0028](../reports/0028-staging-browser-app.md)。専用D1とWorkerは配置済みで、実 URL の書き込みデータは削除APIで除去した。実機と支援技術の証拠は別途未確認である。
