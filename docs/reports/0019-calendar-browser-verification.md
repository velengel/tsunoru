...
## Mutation connection affinity follow-up

[Review 3939865260](https://github.com/velengel/tsunoru/pull/6#discussion_r3939865260) requires closing the interval between identity verification and a mutation. Deterministic HTTP fixtures return a valid identity while handing the listening port to another server. The prior separate-request sequence and Python's ordinary reconnecting HTTPConnection both delivered a mutation to the replacement. After the fixes, both clients fail and the replacement receives zero writes.

Each mutation now shares one physical TCP connection with its own identity GET. Python's BoundHTTPConnection rejects a second connect call; Node's BoundAgent permits one connection and refuses a replacement. A closed connection fails instead of retrying. The browser runner forwards a mutation through that agent and fulfills its route with the real server response. Requests preserve cookies; responses preserve status, bytes and multiple Set-Cookie values. This is an intercepted browser transport to the actual server, not a mocked application response. It requires persistent HTTP/1.1 support; see [ADR 0041](../ADR/0041-bind-verification-writes-to-one-connection.md).

Verification passed: both listener-handover controls, a Node same-socket success/response/cookie check and mismatch control, the complete HTTP lifecycle, 320px/1440px browser creation-to-answer flow, both wrong-database controls, WAL-only preservation, Python shutdown six cases, Node shutdown fourteen cases and outer-driver shutdown ten cases. Local review checked automatic-reconnect behavior against the installed Python and Node clients, Chromium's multiple-cookie fulfillment path, origin checks, body framing, identity size limits and connection cleanup. No application Rust/CSS changed; previous application-test/build evidence remains applicable.

## Exited process-group leader follow-up

[Review 3939917795](https://github.com/velengel/tsunoru/pull/6#discussion_r3939917795) reproduced an owned child surviving Harness.stop after its leader exited. Python and Node cleanup now keep the dedicated group under management until no members remain, with a graceful deadline and bounded KILL fallback independent of leader status. Completed cleanup is idempotent. Python reaps the leader before group probes; a permission failure is treated as completion only when the group is confirmed absent. Node still attempts other owned-resource cleanup if its group cleanup fails.

All four exited-leader fixtures passed: a normal child and a TERM-resistant child in each language. Existing Python six inner interruptions and normal HTTP, Node fourteen inner interruptions, ten outer interruptions, both wrong-database controls and WAL preservation also passed after the final adjustment. Local review checked leader/group distinction, group-scoped signal targets, reaping and completion races, cleanup registration retirement, forced cleanup and the new fixture's own failure paths. See [ADR 0042](../ADR/0042-reclaim-owned-process-groups-through-completion.md).
