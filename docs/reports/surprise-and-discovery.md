...
## 2026-09-07 PR #25 review

- 公開前判断を一つのADRにまとめると、後続Issueの独立した変更単位と不利益の記録が曖昧になるため、運用条件ごとのADR分割が必要だった。

## 2026-09-07 staging deployment

- Wranglerのローカル設定にないリモート認証変数は、通常のdeployで上書き対象になる。既存の認証設定を保持する更新では`--keep-vars`を使い、secret値をローカル設定やコマンドへ出さない。
