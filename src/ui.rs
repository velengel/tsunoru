...
use crate::{
    domain::{
        ...
        CandidateResponseSummary, CandidateSummaryFact, CreatedEvent, EVENT_CANDIDATE_MAX_COUNT,
        EVENT_NAME_MAX_CHARS, EventContinuationCreateInput, EventContinuationPlan,
        EventContinuationPlanInput, EventCreationDraft, EventCreationErrors, LOGIN_ID_MAX_CHARS,
        NewAvailabilityResponseInput, NewResponseCommentInput, ORGANIZER_NOTE_MAX_CHARS,
        OrganizerDecisionInput, OrganizerEventDecision, OrganizerEventSummary,
        OrganizerResponseMatrix, OrganizerSummaryInput, ParticipantResponseMatrix, PublicCandidate,
        PublicEvent, PublicEventDecision, RESPONDENT_COMMENT_MAX_CHARS, RESPONDENT_NAME_MAX_CHARS,
...
fn CandidateDateTimePicker(
    ...
    use_effect(move || {
        spawn(async move {
            let script = "const d = new Date(); const y = d.getFullYear(); const m = String(d.getMonth() + 1).padStart(2, '0'); const day = String(d.getDate()).padStart(2, '0'); dioxus.send(`${y}-${m}-${day}`);";
            if let Some(month) = read_browser_value(script)
                .await
                .and_then(|value| CalendarMonth::from_iso_date(&value))
            {
...
fn format_local_start(local_date: &str, local_time: &str) -> String {
