...
try {
  ...
  const created = await mf.dispatchFetch("http://localhost/api/events", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: "local-event", name: "Local event", organizer_capability: "organizer-capability-1234", response_capability: "response-capability-1234" }) }); if (created.status !== 200 || (await created.json()).id !== "local-event") throw new Error("create failed");
  const answer = await mf.dispatchFetch("http://localhost/api/answers", { method: "POST", headers: { "content-type": "application/json", "x-response-capability": "response-capability-1234" }, body: JSON.stringify({ event_id: "local-event", respondent: "Guest", availability: "available" }) }); if (answer.status !== 200) throw new Error("answer failed");
  const invalid = await mf.dispatchFetch("http://localhost/api/events", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: "", name: "invalid", organizer_capability: "short", response_capability: "short" }) }); if (invalid.status !== 400) throw new Error(`invalid status ${invalid.status}`);
