...
`npm run check` tests the API, sessions, static routing and cleanup against disposable Miniflare instances. It uses neither remote resources nor the native SQLite database. `npm run deploy:check` builds the CSR app and Worker, verifies the external script loader, records asset SHA-256 hashes under `build/`, and bundles without deploying. The generated `public/` directory is ignored and replaced by this build. App changes also require the root checks in `AGENTS.md`.
