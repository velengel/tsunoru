...
Cloudflare の限定試用版は、同じカレンダーと集計表示を使う Dioxus CSR、Rust Worker、専用 D1 で構成する。
試用コードによる入場、イベント作成、共有 URL からの回答、作成したブラウザーでの集計確認が対象で、以下の native 版の全機能は含まない。
ビルドと配置の手順は [限定試用版](cloud/rust-worker/README.md)、実際に確認した範囲は [検証記録](docs/reports/0028-staging-browser-app.md) を参照。
